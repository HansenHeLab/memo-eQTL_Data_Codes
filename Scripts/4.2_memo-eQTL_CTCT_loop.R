...
#############
## 1031 memo-eQTLs
res <- readRDS("./memo-eQTLs_meta_info.RDS")
dim(res)    ## neg cor(CpG, CTCF)
...
### RWPE1 : normal prostate 
{
 
  ### read in CTCF ChIA-PET loop results in rep1
  ...
  pdf("snp-cpg-gene ranges overlap with RWPE1 CTCF loops_label_adj_new.pdf", width = 4, height = 3)
  hist(table(from(snp_gene_ctcf_loop )), breaks = 40, main = "", col = "skyblue",
       xlab = "Number CTCF loops per eSNP-eCpG-eGene" )
  dev.off()
  ...
  length(unique(c(from(cpg_ctcf_left), from(cpg_ctcf_right))))    ## unique sigCpG located in CTCF sits
  ...
  ## distance 
  dis  <- data.frame(ctcf_loop_cpg[from(within_tmp), ], snp_gene_range[to(within_tmp), ])
  pdf("CTCF_loop_within_SNP-Gene_range_RWPE1.pdf", width = 3.5, height = 3.5)
  plot(dis$width, dis$width.1, xlab = "CTCF_loop_width", ylab = "SNP_Gene_distance", pch = 16)
  ...
  M <- matrix(c(cnt$n[2:5]), 2, 2)    ## there's no sigBoth_cross
  ...
  ###################
  {
    idx <- match(rownames(res_both), names(ctcf_loop_cpg))   ## rm duplication
    res_s <- res_both[!is.na(idx), ]
    ...
    g <- ggplot(dat, aes(x = clus1.coef.svar.L, y = clus2.coef.svar.L,
                         color = sigBoth_group, shape = overlap))
    g <- g + geom_point() + labs(x = "meLow-SNP-beta", y = "meHigh-SNP-beta")
    g <- g + geom_hline(yintercept=0, linetype="dashed", color = "gray40")
    g <- g + geom_vline(xintercept=0, linetype="dashed", color = "gray40")
    #g <- g + scale_color_manual(values = c("coral4", "cadetblue", "gray"))
    g <- g + theme_bw()
    g <- ggsave("memo-eQTLs_meLow_vs_meHigh_with_CTCT_loops_RWPE1_sigBoth.pdf", heigh = 3, width = 4)
    
    ## summarise count 
    cnt <- dat %>%  
      group_by(sigBoth_group, overlap) %>% 
      summarise(n = n())
    cnt 
...
## VCaP
{
  ...
  pdf("snp-cpg-gene ranges overlap with VCaP CTCF VeH loops_label_adj_new.pdf", width = 4, height = 3)
  hist(table(from(snp_gene_ctcf_loop )), breaks = 40, main = "", col = "skyblue",
       xlab = "Number CTCF loops per eSNP-eCpG-eGene" )
  dev.off()
  ...
  length(unique(from(cpg_ctcf_left)))
  length(unique(from(cpg_ctcf_right)))
  ## left and righ anchor sites can be overlapped per different loops
  length(unique(c(from(cpg_ctcf_left), from(cpg_ctcf_right))))    ## unique sigCpG located in CTCF sits
  ...
  ## distance 
  dis  <- data.frame(ctcf_loop_cpg[from(within_tmp), ], snp_gene_range[to(within_tmp), ])
  pdf("CTCF_loop_within_SNP-Gene_range_VCaP.pdf", width = 3.5, height = 3.5)
  plot(dis$width, dis$width.1, xlab = "CTCF_loop_width", ylab = "SNP_Gene_distance", pch = 16)
  abline(a = 0 , b = 1, lty = 2)
  dev.off()
  ...
  ## subset
  {
    ...
    pdf("VCaP CTCF loops shared anchor meGpG.pdf", width = 5, height = 3)
    hist(tmp_cnt, breaks = 20,
         xlab = "#CTCF loops share same meGPG", main = "" )
    dev.off()
  ...
  g <- g + scale_color_manual(values = c("coral4", "coral", "cadetblue", "gray"))
  g <- g + theme_classic()
  g <- ggsave("memo-eQTLs_meLow_vs_meHigh_with_CTCT_loops_VCaP.pdf", heigh = 3.5, width = 4.5)
  ...
  ###################
  {
    ...
    #g <- g + scale_color_manual(values = c("coral4", "cadetblue", "gray"))
    g <- g + theme_bw()
    g <- ggsave("memo-eQTLs_meLow_vs_meHigh_with_CTCT_loops_VCaP_sigBoth.pdf", heigh = 3, width = 4)
...
{
  ### read in CTCF HiChIP loop results in VeH treatment
  tmp <- read.table("../Public_Data/3D/22rv1_ctcf_1_loop_hg38.bed")
  ctcf_loop <- GRanges(tmp$V1, IRanges(tmp$V2, tmp$V3))
  
  tmp <- read.table("../Public_Data/3D/22rv1_ctcf_1_loop_left_anchor_hg38.bed")
  ctcf_left <- GRanges(tmp$V1, IRanges(tmp$V2, tmp$V3))
  
  tmp <- read.table("../Public_Data/3D/22rv1_ctcf_1_loop_right_anchor_hg38.bed")
  ctcf_right <- GRanges(tmp$V1, IRanges(tmp$V2, tmp$V3))
  ...
  pdf("snp-cpg-gene ranges overlap with 22Rv1 CTCF loops.pdf", width = 5, height = 3)
  hist(table(from(snp_gene_ctcf_loop )), breaks = 40, main = tt, cex.main = 0.75, sub = st,
       xlab = "#overlapped CTCF loops per snp-cpg-gene range" )
  dev.off()
  
  
  pdf("snp-cpg-gene ranges overlap with 22Rv1 CTCF loops_label_adj.pdf", width = 4, height = 4)
  hist(table(from(snp_gene_ctcf_loop )), breaks = 40, main = "", col = "skyblue",
       xlab = "Number CTCF loops per eSNP-eCpG-eGene" )
  dev.off()
  
  pdf("snp-cpg-gene ranges overlap with 22Rv1 CTCF loops_label_adj_new.pdf", width = 4, height = 3)
  hist(table(from(snp_gene_ctcf_loop )), breaks = 40, main = "", col = "skyblue",
       xlab = "Number CTCF loops per eSNP-eCpG-eGene" )
  dev.off()
  ...
  ## in contrast
  pdf("snp-cpg-gene ranges overlap with 22Rv1 CTCF loops per CTCF.pdf", width = 5, height = 3)
  hist(table(to(snp_gene_ctcf_loop )), breaks = 40,
       xlab = "#overlapped snp-cpg-gene range per CTCF loop", main = "" )
  dev.off()
  ...
  ## CTCF loops with memo-CpG sites
  ctcf_loop_cpg <- ctcf_loop[c(to(cpg_ctcf_left), to(cpg_ctcf_right)), ]
  names(ctcf_loop_cpg) <- c(names(cpg_loci)[from(cpg_ctcf_left)], names(cpg_loci)[from(cpg_ctcf_right)])
  ...
  ## distance 
  dis  <- data.frame(ctcf_loop_cpg[from(within_tmp), ], snp_gene_range[to(within_tmp), ])
  pdf("CTCF_loop_within_SNP-Gene_range_22Rv1.pdf", width = 3.5, height = 3.5)
  plot(dis$width, dis$width.1, xlab = "CTCF_loop_width", ylab = "SNP_Gene_distance", pch = 16)
  abline(a = 0 , b = 1, lty = 2)
  dev.off()
  ...
  if(FALSE){
    ...
    dat <- data.frame(res_s, overlap)
    #saveRDS(dat, file = "memo-eQTLs_meta_info_with_CTCF_loop_VCaP.rds")
    write.csv(dat, file = "memo-eQTLs_meta_info_with_CTCF_loop_22Rv1.csv")
  }
  ...
  ## subset
  {
    ...
    pdf("22Rv1 CTCF loops shared anchor meGpG.pdf", width = 5, height = 3)
    hist(tmp_cnt, breaks = 20,
         xlab = "#CTCF loops share same meGPG", main = "" )
    dev.off()
    ...
    dat <- data.frame(res_s, overlap)
    write.csv(dat, file = "memo-eQTLs_meta_info_with_CTCF_loop_22Rv1.csv")
    saveRDS(dat, file = "memo-eQTLs_meta_info_with_CTCF_loop_22Rv1.rds")
  }
  ...
  g <- g + scale_color_manual(values = c("coral4", "coral", "cadetblue", "gray"))
  g <- g + theme_classic()
  g <- ggsave("memo-eQTLs_meLow_vs_meHigh_with_CTCT_loops_22Rv1.pdf", heigh = 3.5, width = 4.5)
  ...
  ###################
  {
    ...
    #g <- g + scale_color_manual(values = c("coral4", "cadetblue", "gray"))
    g <- g + theme_bw()
    g <- ggsave("memo-eQTLs_meLow_vs_meHigh_with_CTCT_loops_22Rv1_sigBoth.pdf", heigh = 3, width = 4)
